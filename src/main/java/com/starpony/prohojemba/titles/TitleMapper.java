...
public interface TitleMapper {
    //@Select("select titles.id as idTitle, titles.name, titles.cover, types.id as idType, types.viewname from titles left join types on titles.type=types.id")
    @Select(
            ...
            "<where>" +
            "<if test='#{type}!=null'>titles.type=#{type} </if>" +
            //"<if test='#{search}!=null'>titles.name like #{search}</if>" +
            "</where>" +
            "order by titles.name limit #{limit} offset #{offset}")
    @Results(value =
            {
    ...
    List<Title> selectAll(QueryParams queryParams, Where);
